...
/**
 * Class for representing a grid field of tiles, capable of finding paths between tiles in the field.
...
public class AStar {
		...
		for (int k=0;k<currentNeighbors.size();k++){
			...
			if (currentNeighbors.get(k).isClosed() && currentPathIsShorter(currentNeighbors.get(k))){
				currentNeighbors.get(k).setG(currentTile.isDiagonal(currentNeighbors.get(k)) ? 
						currentTile.getG()+1.4 : currentTile.getG()+1);
				...
				currentNeighbors.get(k).setG(currentTile.isDiagonal(currentNeighbors.get(k)) ? currentTile.getG()+1.4: currentTile.getG()+1);
				currentNeighbors.get(k).setParent(currentTile);
	...
	private boolean currentPathIsShorter(AStarTile tile){
		return tile.getG() > (currentTile.isDiagonal(tile) ? currentTile.getG()+1.4 : currentTile.getG()+1);
	}
